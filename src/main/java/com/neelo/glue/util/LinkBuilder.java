...
import com.neelo.glue.st.LinkTool;
...
public class LinkBuilder {
	...
	@Inject
	public LinkBuilder(Provider<HttpServletRequest> requestProvider, RequestResolver resolver) {
	...
	public StringBuilder link(Map<String, Object> def) {
		...
		try {
			if (def.get("url") != null)
				url.append(def.get("url"));
			else if (def.get("path") != null)
				url.append(def.get("path"));
			else if (def.get("name") != null)
				byName(def, url, def.get("name"));
